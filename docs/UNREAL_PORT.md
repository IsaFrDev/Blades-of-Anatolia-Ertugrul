...
- Blender oba sahnasi (`bl_oba_video.py` + fix1-3 + `bl_oba_video2.py`, `oba_scene.blend`): 160 m tepalikli dasht, 8 o'tov (SM_Yurt_Kayi) + 2 chodir halqa (r 11/17 m), olov (shovqinli emissiya + miltillovchi nur), quduq, arava, bochka/yashik/fonar, 70 daraxt (island_tree, archa; r >= 26 m), toshlar, butalar, o't; Nishita kechki osmon; kamera 72 kadr (yuz → tana → aylanish → oba → tepadan). Gotchalar: `import_model` shablon ildizlari (EMPTY) sahnada qoladi va bolalari render bo'ladi — o'chirish shart; `scene.ray_cast` o'tov tomiga uradi — yerga qo'yishda `Terrain.ray_cast` (obyekt lokal) ishlating; Intel iGPU da EEVEE 1280×720/16 sample = 8.5 daqiqa/kadr, 960×540/6 sample ≈ 2 daqiqa/kadr.

## Blender sahnalari -> UE (2026-09-06)
- Ertug'rul + qalqon (Blender `ertugrul_scene.blend`) -> `SM_ErtugrulHero.fbx` -> `/Game/ErtAssets/Chars/Hero/SM_ErtugrulHero1` (154k uchburchak, 3 material). `BuildLandmarks` oba markazida olov yonida joylaydi (statik, 1.85 m; keyin skeletli versiya bilan almashadi).
- `-ErtFly="E,N,Z,pitch,yaw;..." -ErtFlyFrames=72 -ErtFlyStep=0.35`: Catmull-Rom kamera yo'li, HUD siz, har kadr `flyshot/fly_NNN.png`, oxirida chiqadi. Yashirin oynada ishga tushiring (`Start-Process -WindowStyle Hidden`), aks holda oyna yopilsa `ViewportClosed` bilan chiqadi. Video: `ffmpeg -framerate 12 -i seq/%04d.png` (Windows ffmpeg glob qo'llamaydi - kadrlarni qayta raqamlang). Natija: docs/video/ue_oba_flythrough.mp4.
- Blender oba videosi: `bl_oba_frames.py` PNG ketma-ketlik (uzilsa davom etadi) -> ffmpeg. FFMPEG to'g'ridan-to'g'ri yozish uzilsa fayl bo'sh (48 bayt) qoladi.

## Blender asset fabrikasi (2026-09-06)
`bl_asset_factory.py` (headless): 9 ta protsedural PBR material (o't, tuproq, tosh, qum, qor, tosh yo'l, yog'och, pichan, mato) Cycles bilan 2K ga pishiriladi (Diffuse rang + tangent normal) -> `art/blender_tex/` (gitignore, 8-18 MB har biri; 512 px preview `art/blender_tex_preview/`). `tex_seamless.py` (yarim siljitish + 14% chekka cross-fade) -> `seamless/`. `ue_import_tex.py` -> `/Game/ErtAssets/Tex/T_<rol>_D/_N` (landshaft materiali `M_ErtVertexColor` o'sha nomlarni o'qiydi, qayta qurish shart emas). 7 ta oba elementi (ot bog'lash ustuni, oxur, pichan, o'tin uyumi, Qayi bayrog'i, qurol tokchasi, tosh gulxan) -> GLB -> `/Game/ErtAssets/Gen/<nom>`; `ue_fix_gen_mats.py` har slot uchun `M_<nom>_<wood|cloth|hay|rock>` yaratadi; `ErtFab.cpp` Props kalitlari: tether/trough/hay/woodpile/banner/rack/firepit. Gotcha: `ue_import_gen.py` mavjudlik tekshiruvi noto'g'ri yo'lda -> hamma GLB qayta import bo'ladi, shuning uchun `ue_fix_gen_mats.py` + `ue_gen_usage.py` ni har safar qayta ishga tushiring.
- Muhit yaxshilashlari (ro'yxat 1-4, 2026-09-06): landshaft materiali - o't tinti (0.86,0.94,0.72), vertex-rang ta'siri 0.6->0.32, qiyaliklarda tuproq (slope*1.4); `TerrainColor` oba ichida toptalgan tuproq (ObaHalf+14 m, shovqinli o't dog'lari); olov - nur 45 kd, radius 5 m, alanga kichik, tutun 4-6.5 s balandroq; o't teksturasi barg tolalari bilan qayta pishirildi (`ERT_ONLY=grass`); standart vaqt DayT 0.31 (oltin soat), vodiy tumani SecondFog (0.0022, falloff 0.09, 18 m). Skrinshotlar: docs/screens_oba_ground_v2*.png.

## Oba hayoti, suv, ot nomi, kat-sahna kamerasi (ro'yxat 6-9, 2026-09-06)
- `AErtNpc::SetChore(kind, A, B)`: 1 suv tashish (quduq <-> o'tov), 2 o't o'rish (B nuqtada egilib turadi = crouch pozasi), 3 bola (0.62 masshtab, 230 sm/s yugurib tasodifiy nuqtalar). `SpawnNpcs` oxirida 10 ta (4 ayol, 3 o'roqchi, 3 bola). Oba: qozon-ochoq (uch tosh + qora qozon + olov, OE7/ON-7), har 3-o'tov eshigi oldida kigiz gilam (qizil/ko'k).
...
## Realistik ot va kiyim harakati (2026-09-10)
- Blender plash: `bl_cape_cloth.py` - 24x36 grid, yuqori 2 qator "pin", Cloth (mass 0.4, bending 0.15, self-collision), personaj Collision, Wind (600-1400 tebranadi), 48 kadr -> docs/video/blender_cape_cloth.mp4.
- Ot zanjiri: `bl_horse_rig.py` (ERT_HORSE=<glb> yoki placeholder) -> 21 suyakli skelet (pelvis/spine/chest, neck1-2/head, tail1-2, 4 oyoq x upper/lower/foot), ARMATURE_AUTO og'irlik, protsedural aktsiyalar idle/walk/trot/gallop/jump/death -> `D:\Yuklanadiganlar\horse_rig\SK_Horse.fbx`; `ue_import_horse.py` -> `/Game/ErtAssets/Horse/SK_Horse` + `SK_HorseHorseRig_<anim>`; `character.json` horse.mesh/anims shu yo'llarga qo'yildi (placeholder ot bilan sinov; haqiqiy AssetHub oti kelganda ERT_HORSE bilan qayta ishga tushirish kifoya). Old tomon avtomatik aniqlanadi (bosh baland vertexlar), ERT_HORSE_YAW bilan burash mumkin.

## Video mexanikalari (parkour/ot/jang/plash, 2026-09-10)
- Ot bilan bosib o'tish: chavandoz bilan 550+ sm/s da 1.5 m oldindagi dushmanlar (ittifoqchi emas) 18 + tezlik*0.03 zarar oladi, uloqtiriladi (LaunchCharacter), ot 15% sekinlashadi (`AErtHorse::Tick`, 0.12 s tekshiruv).
- Yashirin takedown: dushman sezmagan (`IsAlerted()` false) va orqasidan (<2.1 m, dot < -0.25) urilsa bir zarbada ("Yashirin zarba!"), `StealthKills` hisoblanadi.
- Dinamik plash: 4 bo'g'inli zanjir (`CloakSegs`), tezlik orqaga ko'taradi (55° gacha), tezlanish/tormoz silkitadi, havoda ko'tariladi, shamol/yurish ritmi tebratadi, bo'g'inlar kechikib ergashadi (FInterpTo 9..4.5). Protsedural tanada `Torso` ga, skelet tanada `spine_03` suyagiga (`SkelBuildCloak`, `CloakBaseRot` bilan suyak o'qiga moslash). Qahramonning uchala jangchisida plash yoqilgan (`SetWarrior`).
- Parkour: `MantleMaxHeight` 185 -> 265 sm (baland devor/to'siqlar). Gotcha: skelet tana `SetWarrior` dan oldin quriladi - qayta qurish yo'lida (`if (Skel) {...}`) qilich bilan birga plashni ham qayta qurish kerak.

## Paketlash (.exe) (2026-09-10)
`RunUAT.bat BuildCookRun -project=... -platform=Win64 -clientconfig=Development -build -cook -stage -pak -archive -archivedirectory=D:/Ertugrul_Build`. DefaultGame.ini: `DirectoriesToAlwaysStageAsUFS=(Path="Ertugrul/Data")` (JSON/CSV o'yin vaqtida `ProjectContentDir()` dan o'qiladi - pak ichida bo'lishi shart), `MapsToCook=/Game/Ertugrul/Maps/World`. Gotcha: `ADirectionalLight::GetComponent()` faqat editor buildda bor - o'yin targetida `Cast<UDirectionalLightComponent>(Sun->GetLightComponent())` ishlating (paketlash C2039 bilan to'xtagan edi). VibeUE/MCP pluginlari Editor turida - paketga kirmaydi.
- Paketlash natijasi: `D:\Ertugrul_Build\Windows\Ertugrul.exe` (1.8 GB, Development, pak). Gotcha: cook commandleti ham MCP serverni 8000-portda ochadi; port band bo'lsa (eski `python mcp_py.py` jarayoni) "HttpListener unable to bind" xatosi cook ni "Failure - 1 error" bilan to'xtatadi - paketlashdan oldin 8000-portni bo'shating (`Get-NetTCPConnection -LocalPort 8000`).
- 8000-port: foydalanuvchining `uvicorn server.main:app` ilovasi ham 8000 da - cook commandletida MCP ni o'chirish: `-additionalcookeroptions="-ini:EditorPerProjectUserSettings:[/Script/ModelContextProtocolEngine.ModelContextProtocolSettings]:bAutoStartServer=False"`. Paketga kiritish: `DirectoriesToAlwaysCook` (/Game/ErtAssets, /Game/Characters, /Game/Ertugrul, /Game/Input) - aks holda Fab kutubxonasi 0 asset, Mannequin topilmaydi (protsedural zaxira).
- Paketda Fab kutubxonasi 0 edi: registr asinxron yuklanadi - `AR.IsLoadingAssets()` bo'lsa `WaitForCompletion()`, bo'sh qaytsa `GetAssetsByClass(StaticMesh)` + yo'l filtri zaxirasi (`ErtFab.cpp`). Binar yangilangach `-skipbuild -skipcook -stage -pak -archive` bilan tez qayta paketlash.
- Paketda registr to'liq emas (o'lchandi: jami 1205, /Game/ErtAssets 12 ta; muharrirda 123 mesh). Yechim: `ue_fab_manifest.py` muharrirda `Content/Ertugrul/Data/fab_manifest.json` ga barcha skan yo'llaridagi StaticMesh obyekt yo'llarini yozadi; `ErtFab.cpp` registr natijalarini manifest bilan birlashtiradi (takrorsiz) va `LoadObject` bilan yuklaydi. **Yangi asset import qilgandan keyin manifestni qayta yarating.**

## Kiyim (kaftan) harakati — Blender (2026-09-12)
`bl_kaftan_rig.py`: AI-mesh (Hunyuan) uchun **cloth simulyatsiya ishlamaydi** (nomanifold yupqa qobiq - self-collision portlaydi, bo'laklar uchib ketadi). Buning o'rniga **suyakli tebranish rigi**: bel (0.575H) va etak (0.325H) orasidagi tashqi qobiq (radius > 0.6*rmax, 17.4k vertex) 4 sektorga (old/o'ng/orqa/chap) x 2 bo'g'inga bo'linadi; og'irlik belda 0 -> etakda 1 (smoothstep), sektorlar orasida yumshoq aralashma. Animatsiya: pose suyaklari **dunyo o'qi** atrofida buriladi (`Quaternion(local_axis(bone, world_axis), angle)`, suyak matrix_local inversiyasi orqali) - aks holda har sektorning o'z o'qi tufayli tebranish noto'g'ri tomonga ketadi. Pastki bo'g'in 0.14 s kechikib ergashadi (inersiya). Natija: `docs/video/kaftan_cloth.mp4`, FBX (etak suyaklari + animatsiya): `D:\Yuklanadiganlar\ertugrul_fbx\SK_Ertugrul_Kaftan.fbx` - Mixamo rigi kelganda shu suyaklar tana skeletiga qo'shiladi.
- Kiyim harakati kengaytirildi (`bl_cloth_parts.py`): kaftan etagi (4 sektor x 2 bo'g'in, z 0.325H-0.50H, kamar va xaltacha tepada qoladi), mo'yna yopinchiq (4 sektor x 1 bo'g'in, z 0.70H-0.845H, kichik amplituda 4-11°), yeng uchlari (chap/o'ng, z 0.50H-0.61H, |x| > 0.30 - kamar qini va kaftan yon etagidan tashqarida, qarama-qarshi fazada). `ERT_PROBE=1` mesh z-bandlarini o'lchaydi, `ERT_CUFF=0` yengni o'chiradi. Gotcha: zonalarni faqat z bilan ajratish yetmaydi - qo'llar (|x| > 0.30, |y| < 0.12) etak/mo'yna guruhlaridan chiqarilishi kerak, aks holda kaftan yon etagi va kamar xaltachasi cho'zilib ketadi. Natija: docs/video/kiyim_harakati.mp4, FBX: `SK_Ertugrul_Cloth.fbx`.
