...
void AErtCharacter::UpdateAutoPlay(float Dt)
{
	...
	if (St != EErtMissionState::Inactive && !D->GetEpisodeId().IsEmpty())
	{
		if (AutoEpisode.IsEmpty()) AutoEpisode = D->GetEpisodeId();
		else if (AutoEpisode != D->GetEpisodeId()) { UE_LOG(LogErtugrul, Log, TEXT("[AutoPlay] %s tugadi, keyingi (%s) boshlandi - chiqish (%.0f s, %d teleport)"), *AutoEpisode, *D->GetEpisodeId(), AutoTotalT, AutoTeleports); FPlatformMisc::RequestExit(false); return; }
	}
	...
	else
	{
		for (const FErtObjective& O : D->GetObjectives())
		{
			...
			if (O.Kind == EErtObjKind::Council)
			{
				// Kengash: markerga eng yaqin NPC ga borib gaplashamiz
				TArray<FVector> Ms; D->GetMarkers(Ms);
				const FVector Ref = Ms.Num() ? Ms[0] : (O.Point.IsNearlyZero() ? GetActorLocation() : O.Point);
				TArray<AActor*> Ns; UGameplayStatics::GetAllActorsOfClass(this, AErtNpc::StaticClass(), Ns);
				float Bd = 1e12f; for (AActor* A : Ns) { const float Dd = FVector::DistSquared2D(A->GetActorLocation(), Ref); if (Dd < Bd) { Bd = Dd; Target = A->GetActorLocation(); bHas = true; } }
				if (bHas) break;
			}
